package tst;

import org.openqa.selenium.By;

public class Locators<statis> {
    //Media page
    public static final By LINK_MEDIA = By.xpath("//a[@href='https://romanceabroad.com/media/index']");
    //Registration
    public static final By LINK_REGISTRATION= By.xpath("//button[@id='show-registration-block']");
    //Media search
    public static final By LINK_SEARCH = By.xpath("//a[@href='https://romanceabroad.com/users/search']");
    //How we work locator
    public static final By LINK_WORK = By.xpath("//a[@href='https://romanceabroad.com/content/view/how-it-works']");
    //Pretty women locator
    public static final By LINK_WOMEN = By.xpath("//a[@href='https://romanceabroad.com/users/search']");
    //Photos locator
    public static final By LINK_PHOTOS = By.xpath ("//a[@href='https://romanceabroad.com/media/index']");
    // Gift locator
    public static final By LINK_GIFTS = By.xpath("//a[@href='https://romanceabroad.com/store/category-sweets']");
    //Tour TO Ukraine locator
    public static final By LINK_TOUR = By.xpath("//a[@href='https://romanceabroad.com/store/sweets/20-tour_to_ukraine']");
    //Blog locators
    public static final By LINK_BLOG = By.xpath("//a[@href='https://romanceabroad.com/content/view/blog']");
}
